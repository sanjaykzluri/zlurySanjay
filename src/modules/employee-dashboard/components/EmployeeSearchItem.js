...
export default function EmployeeSearchItem({
	name,
	image,
	categoryName,
	status,
	...
	return (
		<div
			className="search__result__card cursor-pointer"
			onClick={() => {
				history.push({
					pathname: url,
					state: {
						app_in_org: item?.app_in_org,
					},
				});
			}}
		>
			...
			<div className="search__item-subtitle">
				...
				{typeof status === "string" &&
					status?.toLocaleLowerCase() === "inactive" && (
						<div style={{ margin: "0 7px" }}>
							<img alt="inactive" src={inactivecheck} /> Inactive
						</div>
					)}
			</div>
		</div>
